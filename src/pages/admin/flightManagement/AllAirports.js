...
const AllAirports = ({ menuItems, placeholder, name, value, onChange, gap }) => {

    return (
        // <div className="relative ">
        <div className="relative inline-block mt-4 xl:mt-0 md:mt-0 lg:mt-0 sxl:mt-0 text-left ">
            <select onChange={onChange} name={name} defaultValue="" className={`${gap}`} id="grid-state" >
                <option>{placeholder}</option>
                {
                    menuItems.map((item) => {
                        return (
                            <option value={item.code} key={item.code} className="cursor-pointer rounded-lg hover:bg-zinc-200 text-gray-100 hover:font-black font-semibold hover:text-zinc-900 block px-4 py-2 text-md">{item.name}</option>
                        )
                    })
                }
            </select>
        ...
        //         {getAllAirports.value !== "" ? getAllAirports.value : "Options"}
