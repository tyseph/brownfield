...
import { getNewPassword, getOTP } from "../../api/UserDetailsService";
    ...
    getOTP(emailId)
      ...
      .catch((err) => {
        switch (err.response.status) {
          case 400:
            toast.error(
              "No such user exists. Please enter a valid email or register first!",
              {
                position: "bottom-left",
                autoClose: 5000,
                hideProgressBar: false,
                ...
                <div className="mt-6">
                  <span className="block w-full rounded-md shadow-sm">
                    {showSubmit === true ? (
                      <button
                        type="submit"
                        className="flex justify-center w-full px-4 py-2 text-sm font-medium text-white bg-sky-900 border border-transparent rounded-md hover:bg-#1F2937-500 focus:outline-none transition duration-150 ease-in-out"
                      >
                        Re-send OTP
                      </button>
                    ) : (
                ...
                {showSubmit == true && showPassword == false ? (
                  <div>
                    <form onSubmit={submitHandler}>
                      <div>
                        <label
                          htmlFor="otp"
                          className="block text-sm font-medium leading-5 pt-4"
                        >
                          {" "}
                          OTP{" "}
                        </label>
                  ...
                  <div>
                    <form onSubmit={submitNewPasswordHandler}>
                      <div>
                        <label className="block text-sm font-medium leading-5 pt-4">
                          {" "}
                          New Password{" "}
                        </label>
                        <div className="mt-1 rounded-md shadow-sm">
